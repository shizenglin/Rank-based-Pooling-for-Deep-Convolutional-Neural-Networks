...
LayerParameter SPPLayer<Dtype>::GetPoolingParam(const int pyramid_bin,
      const int bottom_h, const int bottom_w, const SPPParameter spp_param) {
  LayerParameter pooling_param;
  //int num_bins = pow(2, pyramid_level);
  int num_bins = pyramid_bin;
  ...
  int stride_h = bottom_h / num_bins;
  ...
  int stride_w = bottom_w / num_bins;
  if(bottom_h==11 && num_bins==4)
  {
      kernel_h=4;
      stride_h=3;
      kernel_w=4;
  ...
  case SPPParameter_PoolMethod_STOCHASTIC:
    ...
    break;
 case SPPParameter_PoolMethod_RANKSTOCHASTIC:
    pooling_param.mutable_pooling_param()->set_pool(
...
void SPPLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  pyramid_height_.clear();
  std::copy(spp_param.pyramid_height().begin(),
      spp_param.pyramid_height().end(),
      std::back_inserter(pyramid_height_));
  //pyramid_height_ = spp_param.pyramid_height();
  pyramid_height_size = pyramid_height_.size();
  ...
  if (pyramid_height_size == 1) {
    // pooling layer setup
    LayerParameter pooling_param = GetPoolingParam(pyramid_height_[0], bottom_h_, bottom_w_,
        spp_param);
  ...
  for (int i = 0; i < pyramid_height_size; i++) {
  ...
  for (int i = 0; i < pyramid_height_size; i++) {
    ...
    LayerParameter pooling_param = GetPoolingParam(
        pyramid_height_[i], bottom_h_, bottom_w_, spp_param);
...
void SPPLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  vector<bool> concat_propagate_down(pyramid_height_size, true);
